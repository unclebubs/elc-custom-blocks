...
class PodcastYearNavigationTest extends WP_UnitTestCase
{
  ...
  protected function setUp(): void
  {
    ...
    $post2_id = $this->factory->post->create([
      ...
      'post_type' => 'podcast',
      'post_status' => 'publish',
      'post_date' => '2024-01-01 12:00:00', // Local time
    ]);
    ...
    wp_set_object_terms($post3_id, [$this->parent_term_id, $this->child1_term_id], 'podcast-category');

    $post4_id = $this->factory->post->create([
      'post_title' => 'Dummy Podcast Post 4',
      'post_type' => 'podcast',
      'post_status' => 'publish',
      'post_date' => '2022-01-03 12:00:00', // Local time
    ]);
    ...
    $post5_id = $this->factory->post->create([
      ...
      'post_type' => 'podcast',
      'post_status' => 'publish',
      'post_date' => '2021-05-03 12:00:00', // Local time
    ]);
    ...
    $post6_id = $this->factory->post->create([
      ...
      'post_type' => 'podcast',
      'post_status' => 'publish',
      'post_date' => '2020-01-03 12:00:00', // Local time
    ]);
    wp_set_object_terms($post6_id, [$this->parent_term_id, $this->child1_term_id], 'podcast-category');

    $post7_id = $this->factory->post->create([
      'post_title' => 'Dummy Podcast Post 7',
      'post_type' => 'podcast',
      'post_status' => 'publish',
      'post_date' => '2019-01-03 12:00:00', // Local time
    ]);
    wp_set_object_terms($post7_id, [$this->parent_term_id, $this->child1_term_id], 'podcast-category');
  }
  ...
  public function test_all_pocasts_are_listed()
  {
    global $wp_query;
    ...
    // Call the function
    $output = render_podcast_years([], '');

    $count = substr_count($output, 'nav-item');

    $this->assertEquals(6 + 1, $count, 'The string "nav-item" should occur 7 times');
    $years = [2019, 2020, 2021, 2022, 2024, 2024, 'All'];
    foreach ($years as $year) {
      $this->assertStringContainsStringIgnoringCase('data-year="' . $year . '"', $output, 'The output should should containe the year ' . $year);
    }
  }
}
